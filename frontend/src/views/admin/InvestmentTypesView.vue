...
const items = ref<InvestmentType[]>([])
const showForm = ref(false)
const editingId = ref<string | null>(null)
const form = reactive({ name: '', description: '', isActive: true })

const columns: DataTableColumn<InvestmentType>[] = [
...
async function load() {
  const { data } = await api.get<InvestmentType[]>('/admin/investment-types')
  items.value = data
}
...
function openEdit(item: InvestmentType) {
...
async function save() {
  try {
    if (editingId.value) await api.put(`/admin/investment-types/${editingId.value}`, form)
    else await api.post('/admin/investment-types', form)
...
<template>
  <div class="page">
    <div class="page-header">
      <div><h1>Tipos de investimento</h1><p class="muted">LCI, LCA, CDB e outros tipos cadastráveis.</p></div>
